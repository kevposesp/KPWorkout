...
const Chart = () => {
    ...
    const { productsChart, addProduct, removeProduct, removeLine, getProductsChart } = useChart();
            ...
            {productsChart.length === 0 ? (<div className="text-center text-gray-600">No products in Chart</div>)
                : productsChart.map(product => (
                    <div className="block md:flex items-center justify-between border gap-6 p-4 border-gray-200 rounded" key={product.id}>
                        ...
                        <div className="w-full md:w-1/3 my-2 md:m-0">
                        ...
                        <div className="text-primary text-lg font-semibold my-2 md:m-0">{product.price} €</div>

                        <form className="md:max-w-xs mx-auto my-2 md:m-0">
                        ...
                        <div className='mt-4 md:mt-0'>
                            <a onClick={() => removeLine(product.id)}
                                className='px-6 py-2 text-center text-sm text-white bg-red-700 border border-red-400 rounded hover:bg-transparent hover:text-red-700 transition uppercase font-roboto font-medium ms-auto md:ms-2'>
